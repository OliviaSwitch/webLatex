...
当 $f(\boldsymbol{x})$ 在 $\bar{\boldsymbol{x}}$ 处可微或二次可微时，

\begin{equation}
    \nabla f(\bar{\boldsymbol{x}}) = (f_{x_1}(\bar{\boldsymbol{x}}), f_{x_2}(\bar{\boldsymbol{x}}),\cdots,f_{x_n}(\bar{\boldsymbol{x}}))^T = (\frac{\partial f}{\partial x_1}, \frac{\partial f}{\partial x_2}, \cdots, \frac{\partial f}{\partial x_n})^T
    \label{equ:grad}
\end{equation}
...
\begin{equation}
    ...
    =\begin{pmatrix}
        f_{x_1x_1}(\bar{\boldsymbol{x}}) & f_{x_1x_2}(\bar{\boldsymbol{x}}) & \cdots & f_{x_1x_n}(\bar{\boldsymbol{x}}) \\
        f_{x_2x_1}(\bar{\boldsymbol{x}}) & f_{x_2x_2}(\bar{\boldsymbol{x}}) & \cdots & f_{x_2x_n}(\bar{\boldsymbol{x}}) \\
        \vdots                           & \vdots                           & \ddots & \vdots                           \\
        f_{x_nx_1}(\bar{\boldsymbol{x}}) & f_{x_nx_2}(\bar{\boldsymbol{x}}) & \cdots & f_{x_nx_n}(\bar{\boldsymbol{x}})
    \end{pmatrix}
    =\begin{pmatrix}
        \cfrac{\partial^2 f}{\partial x_1^2}           & \cfrac{\partial^2 f}{\partial x_2\partial x_1}  & \cdots & \cfrac{\partial^2 f}{\partial x_n\partial x_1} \\
        \cfrac{\partial^2 f}{\partial x_1\partial x_2} & \cfrac{\partial^2 f}{\partial x_2^2}            & \cdots & \cfrac{\partial^2 f}{\partial x_n\partial x_2} \\
        \vdots                                         & \vdots                                          & \ddots & \vdots                                         \\
        \cfrac{\partial^2 f}{\partial x_1\partial x_n} & \cfrac{\partial^2 f}{\partial x_2 \partial x_n} & \cdots & \cfrac{\partial^2 f}
        {\partial x_n^2}
    \end{pmatrix}
    \label{equ:Hesse matrix}
\end{equation}
简而言之，式(\ref{equ:Hesse matrix})即为式(\ref{equ:grad})逐行关于参数（即$x_1, x_2, x_3, \cdots$）求偏导。

根据数学分析的有关理论，有下列泰勒展开式。当 $f(\boldsymbol{x})$ 在 $\bar{\boldsymbol{x}}$ 处二次可微时，有

一阶泰勒展开式：
\begin{equation}
    f(\boldsymbol{x}) = f(\bar{\boldsymbol{x}}) + [\nabla f(\bar{\boldsymbol{x}})]^T(\boldsymbol{x}-\bar{\boldsymbol{x}}) + o(\Vert\boldsymbol{x}-\bar{\boldsymbol{x}}\Vert)
    \label{equ:1st Taylor}
\end{equation}

二阶泰勒展开式：
\begin{equation}
    f(\boldsymbol{x}) = f(\bar{\boldsymbol{x}}) + [\nabla f(\bar{\boldsymbol{x}})]^T(\boldsymbol{x}-\bar{\boldsymbol{x}}) + \frac{1}{2}(\boldsymbol{x}-\bar{\boldsymbol{x}})^T\nabla^2f(\bar{\boldsymbol{x}})(\boldsymbol{x}-\bar{\boldsymbol{x}}) + o(\Vert\boldsymbol{x}-\bar{\boldsymbol{x}}\Vert^2)
    \label{equ:2nd Taylor}
\end{equation}

不过实际考试中，一般会将最后一项$o()$忽略，并将等号改为约等于号$\approx$。

另外，这里给出特殊函数的一阶二阶导：

\begin{enumerate}
    \item 线性函数：$f(\boldsymbol{x})=\boldsymbol{c}^T\boldsymbol{x}+d$，则$\nabla f(\boldsymbol{x})=\boldsymbol{c}$，$\nabla^2 f(\boldsymbol{x})=0$
    \item 二次函数：$f(\boldsymbol{x})=\frac{1}{2}\boldsymbol{x}^T\boldsymbol{Qx}+\boldsymbol{c}^T\boldsymbol{x}+d$，则$\nabla f(\boldsymbol{x})=\boldsymbol{Qx}+\boldsymbol{c}$，$\nabla^2 f(\boldsymbol{x})=\boldsymbol{Q}$
    \item 向量值线性函数：$F(\boldsymbol{x})=\boldsymbol{Ax}+\boldsymbol{b}$，则$F^{\prime}(\boldsymbol{x})=\frac{\partial F}{\partial \boldsymbol{x}}=\boldsymbol{A}^T$
\end{enumerate}

利用复合函数求导法则，注意向量运算。设 $\Phi(\boldsymbol{x})=f(\boldsymbol{Ax}+\boldsymbol{b})$ ，则
$$
    \Phi^{\prime}=\boldsymbol{A}^T\nabla f(\boldsymbol{Ax}+\boldsymbol{b})
$$
$$
    \Phi^{\prime\prime}=\boldsymbol{A}^T\nabla^2 f(\boldsymbol{Ax}+\boldsymbol{b})\boldsymbol{A}
$$
...
\begin{enumerate}
    ...
    \item 求出\textbf{检验数}$\boldsymbol{\sigma}_j=c_j-\sum\limits_{i=1}^mc_{n+i}a_{ij}$，可以通俗理解为$\sigma_1=$第一行中的$c_1$减去（$c_B$的每一项与$c_1$列对应行的$a_1$相乘的求和）。\\
...
\begin{figure}[ht]
    \centering
    \input{SteepestDescentMethodFlow.tex}
    \caption{最速下降法流程}
...
\begin{figure}[ht]
    \centering
    \input{UnconstrainedNewtonMethodFlow.tex}
    \caption{牛顿法流程}
    \label{fig:flow of unconstrained Newton's method}
\end{figure}

% 需要注意的是，在实际计算中，$\nabla^2f(\boldsymbol{x}^{(k)})$ 的正定性不易保证，因此需要对其进行修正。
...
\begin{figure}
    \centering
    \input{ConjugateGradientMethodFlow.tex}
    \caption{共轭梯度法流程}
...
\begin{figure}[ht]
    \centering
    \input{VariableMetricMethodFlow.tex}
    \caption{变尺度法流程}
...
% \section{DFP ( Davidon(1959),Fletcher and Powell(1963) ) 法和BFGS ( Broyden(1970), Fletcher(1970),Goldfarb(1970),Schanno(1970) ) 法}
...
最优性条件即%为Kuhn-Tucker条件
\begin{equation}
    \nabla f(\boldsymbol{x}^*) = -\sum_{i=1}^{l}\boldsymbol{v}_i^*\nabla h_i(\boldsymbol{x}^*)
    % \label{equ:Kuhn-Tucker condition}
\end{equation}

% 以二维情况为例，当 $l=1$ 时，有最优性条件如图所示
% 在 $\boldsymbol{x}^*$ 点，$\nabla f(\boldsymbol{x}^*)$ 与 $\nabla h(\boldsymbol{x}^*)$ 共线，沿函数下降方向的点不可行，故 $\boldsymbol{x}^*$ 为l.opt.；在 $\bar{\boldsymbol{x}}$ 点，$\nabla f(\bar{\boldsymbol{x}})$ 在 $\nabla h(\bar{\boldsymbol{x}})=0$ 曲线的$\bar{\boldsymbol{x}}$点的切线方向上有投影，即存在下降并线性可行方向，故 $\bar{\boldsymbol{x}}$ 不是l.opt.。这里线性可行方向指切线方向。

% \begin{figure}[ht]
%     \centering
%     % \input{Kuhn-TuckerCondition.tex}
%     \begin{tikzpicture}[line width=1pt]
%         \draw (0,0) arc (190: 350: 4 );
%         \draw[red] (0, 0) -- (4.5, -1.5); % 切线
%     \end{tikzpicture}
%     \caption{Kuhn-Tucker条件}
%     \label{fig:Kuhn-Tucker condition}
% \end{figure}
...
\begin{theorem}[最优性必要条件；K-T条件]
    ...
    \begin{gather}
        % \nabla f(\boldsymbol{x}^*)+\frac{\partial\boldsymbol{g}(\boldsymbol{x}^*)}{\partial\boldsymbol{x}}\boldsymbol{u}^* =\boldsymbol{0} \\
        \nabla f(\boldsymbol{x}^*)+\sum_{i=1}^{m}u_i^*\nabla g_i(\boldsymbol{x}^*)=\boldsymbol{0} \\
        \boldsymbol{u}^*\ge\boldsymbol{0}                                                         \\
        \boldsymbol{u}^{*T}\boldsymbol{g}(\boldsymbol{x}^*)=0\hbox{（互补松弛条件）}
    \end{gather}
    \label{thm:Kuhn-Tucker condition of inequality constraints}
\end{theorem}
...
考虑一般约束问题 $(f\boldsymbol{gh})$，结合定理 \ref{thm:Kuhn-Tucker condition of inequality constraints} ，可以平行得到下面的定理。

\begin{theorem}
    考虑问题$(f\boldsymbol{gh})$, $\boldsymbol{x}^* \in S = \{\boldsymbol{x}|\boldsymbol{g}(\boldsymbol{x}) \le 0, \boldsymbol{h}(\boldsymbol{x}) = 0\}$, $I$ 为作用集。

    设 $g_i(\boldsymbol{x})(i \in I)$ 在 $\boldsymbol{x}^*$ 点可微, $g_i(\boldsymbol{x})(i \notin \boldsymbol{I})$ 在 $\boldsymbol{x}^*$ 连续, $h_j$ ($j=1,2,\cdots,i$)在 $\boldsymbol{x}^*$ 的某邻域内连续可微。
    再设约束规格$(CQ)\{\nabla g_i(\boldsymbol{x}^*),i\in I,\nabla h_j(\boldsymbol{x}^*),j=1,2,\cdots,l\}$线性无关。那么存在 $u_i\ge0,i\in I,v_j,j=1,2,\cdots,l$ 使得
    \begin{equation}
        \nabla f(\boldsymbol{x}^*)+\sum_{i\in I}u_i\nabla g_i(\boldsymbol{x}^*)+\sum_{j=1}^{l}v_j^*\nabla h_j(\boldsymbol{x}^*)=\boldsymbol{0}
    \end{equation}
    如果进一步有 $g_i(\boldsymbol{x}),i\notin I$ 在 $\boldsymbol{x}^*$ 点也可微，那么下列系统成立
    \begin{gather}
        \nabla f(\boldsymbol{x}^*)+\sum_{i=1}^{m}u_i^*\nabla g_i(\boldsymbol{x}^*)+\sum_{j=1}^{l}v_j^*\nabla h_j(\boldsymbol{x}^*)=\boldsymbol{0} \\
...
一种把约束变化为标准形式的线性约束问题的算法。问题为
\begin{equation}
    (f\boldsymbol{Ab})=\begin{cases}
        \min & f(\boldsymbol{x})               \\
        s.t. & \boldsymbol{Ax}=\boldsymbol{b}  \\
             & \boldsymbol{x}\ge\boldsymbol{0}
    \end{cases}
    \label{equ:standard form of linear constraint problem}
\end{equation}
其中 $\boldsymbol{A}$为$m\times n$矩阵，$\boldsymbol{b}\in\mathbf{R}^m$。

对问题 \ref{equ:standard form of linear constraint problem} 建立非退化假设：
\begin{enumerate}
    \item $\boldsymbol{A}$ 的任意 $m$ 列线性无关；
    \item 任意可行解 $\boldsymbol{x}\in S$ 至少有 $m$ 个正分量。
\end{enumerate}

在非退化假设下，$\forall \boldsymbol{x}\in S$，存在分解 $\boldsymbol{A}=(\boldsymbol{B},\boldsymbol{N})$，使得 $\boldsymbol{x}=\begin{pmatrix}\boldsymbol{x}_{\boldsymbol{B}} \\ \boldsymbol{x}_{\boldsymbol{N}} \end{pmatrix}$，其中 $\boldsymbol{x}_{\boldsymbol{B}}>\boldsymbol{0},\boldsymbol{x}_{\boldsymbol{N}}\ge\boldsymbol{0}$，称 $\boldsymbol{x}_{\boldsymbol{B}}$ 为基变量，$\boldsymbol{x}_{\boldsymbol{N}}$ 为非基变量。

相应的，$\nabla f(\boldsymbol{x})=\begin{pmatrix}
        \nabla_{\boldsymbol{B}}f(\boldsymbol{x}) \\
        \nabla_{\boldsymbol{N}}f(\boldsymbol{x})
    \end{pmatrix}$，称 $\boldsymbol{r}_N^T=\nabla_{\boldsymbol{N}}f^T(\boldsymbol{x})-\nabla_{\boldsymbol{B}}f^T(\boldsymbol{x})\boldsymbol{B}^{-1}\boldsymbol{N}$ 为既约梯度。

\begin{figure}[ht]
    \centering
    \input{ReducedGradientMethodFlow.tex}
    \caption{既约梯度法流程}
    \label{fig:flow of reduced gradient method}
\end{figure}
