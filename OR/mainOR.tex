...
理想的收敛性概念一般是指：设 $\boldsymbol{x}^*$ 为问题 $(fS)$ 的全局最优解。当 $\boldsymbol{x}^* \in \{\boldsymbol{x}^{(k)}\}$ 或 $\boldsymbol{x}^{(k)} \ne \boldsymbol{x}^*$ 对于所有 $k$ 成立，并且满足 $\lim_{k \rightarrow \infty} \|\boldsymbol{x}^{(k)} - \boldsymbol{x}^*\| = 0$ 时，称算法收敛到最优解 $\boldsymbol{x}^*$。

\subsection{收敛准则}

\begin{enumerate}
    \item $ ||\boldsymbol{x}^{(k+m)}-\boldsymbol{x}^{(k)}|| < \varepsilon $
